...
uv sync 
uv run netmhc --peptides_filepath <filepath> --alleles_filepath <filepath> --mhc_class [-mhc] <class>
```
...
```sh
uv run netmhc --peptides_filepath <filepath> --alleles_filepath <filepath> --mhc_class [-mhc] <class> --save_filepath [-sf] <filepath>
```
The current behaviour supports saving to CSV. Use `uv run netmhc --help` to see more options.
